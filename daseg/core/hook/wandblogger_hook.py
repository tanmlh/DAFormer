...
class MMSegWandbHook(WandbLoggerHook):
    ...
    def __init__(self,
        ...
        self.test_fn = None
        self.log_data_table=log_data_table
    ...
    def before_run(self, runner):
        ...
        if self.log_evaluation and self.log_data_table:
    ...
    def _add_ground_truth(self, runner):
        ...
        # from daseg.datasets.pipelines import LoadImageFromFile, ClipNormalize, MultiDomainClipNormalize, Compose
        from daseg.datasets.pipelines import LoadImageFromFile, Compose

        img_loader = []
        for t in self.val_dataset.pipeline.transforms:
            if isinstance(t, LoadImageFromFile):
                img_loader.append(t)
            # elif isinstance(t, ClipNormalize):
            #     img_loader.append(t)
            # elif isinstance(t, MultiDomainClipNormalize):
            #     img_loader.append(t)
        ...
        for idx in self.eval_image_indexs:
            ...
            # img_prefix = self.val_dataset.data_root
            img_prefix = None
            if hasattr(self.val_dataset, 'img_dir') and self.val_dataset.img_dir is not None:
                img_prefix = self.val_dataset.img_dir
            ...
            # if self.val_dataset.reduce_zero_label:
            #     seg_mask[seg_mask == 0] = 255
            #     seg_mask = seg_mask - 1
            #     seg_mask[seg_mask == 254] = 255
