...
from daseg.models.decode_heads.isa_head import ISALayer
...
class DAFormerHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        n, _, h, w = x[-1].shape
        # for f in x:
        #     mmcv.print_log(f'{f.shape}', 'daseg')

        os_size = x[0].size()[2:]
        _c = {}
        for i in self.in_index:
            # mmcv.print_log(f'{i}: {x[i].shape}', 'daseg')
            _c[i] = self.embed_layers[str(i)](x[i])
            if _c[i].dim() == 3:
                _c[i] = _c[i].permute(0, 2, 1).contiguous()\
                    .reshape(n, -1, x[i].shape[2], x[i].shape[3])
            # mmcv.print_log(f'_c{i}: {_c[i].shape}', 'daseg')
            if _c[i].size()[2:] != os_size:
                # mmcv.print_log(f'resize {i}', 'daseg')
                _c[i] = resize(
                    _c[i],
                    size=os_size,
                    mode='bilinear',
